...
class WegaVehicleApp extends StatefulWidget {
  const WegaVehicleApp({super.key});

  @override
  State<WegaVehicleApp> createState() => _WegaVehicleAppState();
}

class _WegaVehicleAppState extends State<WegaVehicleApp> {
  ...
  Future<void> _initializeStartupServices() async {
    ...
    try {
      ...
      startupMessage =
          'Firebase nie jest skonfigurowane. Logowanie będzie dostępne po poprawieniu konfiguracji.';
    ...
    try {
      ...
      final notificationMessage =
          'Powiadomienia lokalne nie zostały uruchomione. Pozostałe funkcje aplikacji są dostępne.';
      startupMessage = startupMessage == null
          ? notificationMessage
          : '$startupMessage\n$notificationMessage';
  ...
  Widget _buildHome() {
    if (!_firebaseReady) {
      return LoginScreen(
        firebaseReady: false,
        startupMessage: _startupFinished
            ? _startupMessage
    ...
    return StreamBuilder<User?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return LoginScreen(
            firebaseReady: false,
            startupMessage: _startupMessage ?? 'Sprawdzanie sesji...',
          );
        }
        return snapshot.hasData
            ? const HomeScreen()
            : LoginScreen(startupMessage: _startupMessage);
      },
    );
  }
}
...
class LoginScreen extends StatefulWidget {
  const LoginScreen({super.key, this.firebaseReady = true, this.startupMessage});
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _signIn() async {
    if (!widget.firebaseReady) {
      setState(() {
        _message = widget.startupMessage ?? 'Logowanie chwilowo niedostępne.';
      });
      return;
    }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            FilledButton(
              onPressed: _loading || !widget.firebaseReady ? null : _signIn,
              child: Text(_loading ? 'Logowanie...' : 'Zaloguj'),
            ),
            if (widget.startupMessage != null) ...[
              const SizedBox(height: 12),
              Text(widget.startupMessage!, textAlign: TextAlign.center),
            ],
            if (_message != null) ...[
              const SizedBox(height: 12),
