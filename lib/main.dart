...
const AndroidNotificationChannel _operationChannel = AndroidNotificationChannel(
  'vehicle_operations',
  'Operacje pojazdów',
...
Future<void> _initializeNotifications() async {
  ...
  final androidNotifications = _notifications.resolvePlatformSpecificImplementation<
      AndroidFlutterLocalNotificationsPlugin>();
  await androidNotifications?.createNotificationChannel(_operationChannel);
...
class _OperationScreenState extends State<OperationScreen> {
  ...
  Future<void> _scheduleOperationNotifications(
    ...
    try {
      ...
      final plannedEndAt = DateTime.parse(operation['plannedEndAt'] as String);
      if (type == 'obsługa') {
        await _scheduleNotification(
          id: _operationNotificationId(operation, 'warningNotificationId'),
          title: 'Obsługa pojazdu',
          body: 'Pozostało 5 minut do zakończenia obsługi.',
          when: startedAt.add(const Duration(minutes: _serviceWarningMinutes)),
        );
      }
      await _scheduleNotification(
        id: _operationNotificationId(operation, 'completionNotificationId'),
        title: type == 'ładowanie' ? 'Ładowanie zakończone' : 'Obsługa zakończona',
        body: type == 'ładowanie'
      ...
      operation['notificationsScheduled'] = false;
    }
  }

  Future<void> _cancelOperationNotifications(
    Map<String, dynamic> operation,
  ) async {
  ...
  Future<void> _restoreOperations() async {
    ...
    for (final operation in _activeOperations) {
      unawaited(_scheduleOperationNotifications(operation));
    }
    await _syncPending();
  }
...
class _ScannerScreenState extends State<ScannerScreen> {
  ...
  String? _normalizeVin(String text) {
    final normalized = text.replaceAll(RegExp(r'[\s-]+'), '').toUpperCase();
    final match = RegExp(r'(?<![A-Z0-9])[A-HJ-NPR-Z0-9]{17}(?![A-Z0-9])')
        .firstMatch(normalized);
    return match?.group(0);
  }

  bool _isVin(String text) => _normalizeVin(text) == text.toUpperCase();

  Future<void> _handleDetected(String value, String type) async {
    final normalizedValue = _normalizeVin(value) ?? value.trim().toUpperCase();
    if (normalizedValue.isEmpty || _handlingResult) return;
    final now = DateTime.now();
    if (_lastValue == normalizedValue &&
        _lastReadAt != null &&
        now.difference(_lastReadAt!) < const Duration(seconds: 2)) {
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Odczytano'),
        content: Text('Odczytano: $normalizedValue'),
        actions: [
          TextButton(
    ...
    if (!mounted) return;
    if (confirmed == true) {
      Navigator.of(context).pop(ScanResult(normalizedValue, type));
      return;
    }
  ...
  Future<void> _readText(BarcodeCapture capture) async {
    if (_processingText || capture.image == null || capture.size == null) return;
    ...
    try {
      ...
      final vin = _normalizeVin(text.text);
      if (vin != null) {
        await _handleDetected(vin, 'VIN / OCR');
      }
    } catch (_) {
      // OCR failures should not stop barcode scanning.
    } finally {
  ...
  void _onDetect(BarcodeCapture capture) {
    for (final barcode in capture.barcodes) {
      ...
      if (value != null && value.trim().isNotEmpty) {
        unawaited(_handleDetected(value, _type(barcode, value)));
        return;
      }
    }
            ...
            child: Padding(
              ...
              child: Card(
                child: Padding(
                  ...
                  child: Text(
                    'Skieruj aparat na QR, Code 128, GS1-128, EAN-128 albo numer VIN.',
                  ),
                ),
              ),
            ),
